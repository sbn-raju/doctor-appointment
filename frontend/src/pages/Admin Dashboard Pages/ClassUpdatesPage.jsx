  ...
  const ClassUpdatesPage = () => {
    ...
    const classDetailsMutation = useMutation({
      ...
      onSuccess:async()=>{
        await queryClient.invalidateQueries({queryKey:['upcomingClassDetails']}) ;
      }
    })
    ...
    const handleTerminateClick = ()=>{
      const result = toast.custom((t)=><ClassToaster t={t} message={"Class Terminating"} subMessage={"Are you sure you want to proceed with terminating the class?"} onAgree={handleTerminateAgree} onDisagree={handleTerminateDisagree}/>)
      console.log(result)
    }
    ...
    const stopBooking = useMutation({
      ...
      onSuccess:async()=>{
        await queryClient.invalidateQueries({queryKey:['ongoingClassDetails']})
        await queryClient.invalidateQueries({queryKey:['upcomingClassDetails']})
      }
    })
    ...
    const handleStopBookingClick = ()=>{
      toast.custom((t)=><ClassToaster t={t} message={"Class Stop Booking"} subMessage={"Are you sure you want to stop accepting bookings for this class?"} onAgree={handleStopBookingAgree} onDisagree={handleStopBookingDisAgree}/>)
    }
                  ...
                  <div className='w-full'>
                    <form onSubmit={handleSubmit(handleAddClassLink)}>
                      ...
                      <div className='w-full flex justify-between mt-2'>
                        <CommonButton className='text-red-500 border-[1px] border-white hover:border-red-500 rounded-xl py-2 px-4 hover:bg-red-100' type='button' onClick={handleTerminateClick}> <DangerousIcon/>&nbsp;Terminate Class</CommonButton>
                        <button className='bg-green-600  text-white px-4 py-2 font-medium rounded-xl hover:bg-green-800 flex flex-row justify-center' type='submit'><AddLinkIcon/>&nbsp;Add&nbsp;Link</button>
                      </div>
                    </form>
                  </div>
                </div>
                ...
                <div className='p-8 py-4'>
                  ...
                  <div className='w-full mt-8'>
                    <div className='w-full flex justify-between mt-2'>
                      ...
                      <button className='bg-green-600  text-white px-4 py-2 font-medium rounded-xl hover:bg-green-800 flex flex-row justify-center' onClick={handleStopBookingClick}>Stop Bookings</button>
                    </div>
                  </div>
                </div>
              </div>
