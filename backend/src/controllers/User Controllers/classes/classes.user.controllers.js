...
const setClassBooking = async (req,res)=>{
    ...
    try {
        ...
        const expectedSignature = crypto
          .createHmac('sha256', process.env.RAZORPAY_KEY_SECRET).update(body.toString()).digest('hex');
    
        if (expectedSignature === razorpay_signature) {
            const class_idQuery = "SELECT id FROM active_class_master ORDER BY createdAt DESC LIMIT 1"
            ...
            const setClassBookingQuery = "INSERT INTO user_class_bookings (class_id, name, email, whatsapp, payment_status, city) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *"
            const setClassBookingValues = [class_id, name, email, whatsapp, payment_status, city] 
            try{
                 const setClassBooking = await pool.query(setClassBookingQuery, setClassBookingValues);
